...
def matrix_nms(bboxes,
    ...
    **Matrix NMS**
    ...
    if HAVE_PIR and in_dynamic_or_pir_mode():
        attrs = (float(score_threshold), int(nms_top_k), int(keep_top_k),
                 float(post_threshold), bool(use_gaussian), float(gaussian_sigma),
                 int(background_label), bool(normalized))
